...
export default function AdminModal({ isOpen, onClose, matches, onUpdate }) {
  ...
  const handleScoreSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`/api/admin/update-score/${selectedMatch}`, 
        { scoreA: Number(scoreA), scoreB: Number(scoreB) },
        { headers: { 'x-admin-key': adminKey } }
      );
      setStatusMsg(`Match ${selectedMatch} updated successfully!`);
      onUpdate();
    } catch (err) {
  ...
  const handleInitBracket = async () => {
    if (!window.confirm("Initialize or reset entire bracket with registered teams?")) return;
    try {
        ...
        {!authorized ? (
          ...
          <div className="space-y-6">
            ...
            <form onSubmit={handleScoreSubmit} className="space-y-3">
              <h3 className="text-sm font-bold text-white">Update Best-of-3 Score</h3>
              <div>
                ...
                >
                  ...
                  {matches.map(m => (
                    <option key={m.matchCode} value={m.matchCode}>
                      {m.matchCode}: {m.teamA?.name || 'TBD'} vs {m.teamB?.name || 'TBD'} ({m.status})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-slate-400">Team A Score (Max 2)</label>
                  <input 
                ...
                <div>
                  <label className="text-xs text-slate-400">Team B Score (Max 2)</label>
                  <input 
                    type="number" min="0" max="2" 
                    className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-2 text-white outline-none focus:border-cyan-400 mt-1"
                    value={scoreB}
