...
export default function MatchCard({ match, isAdmin, onScoreChange, onSelectTeam }) {
  ...
  return (
    <div className={`w-64 sm:w-72 bg-slate-900/95 border rounded-xl overflow-hidden shadow-lg transition duration-200 ${
      isLive 
      ...
      <div className="bg-slate-950/90 px-3 py-1.5 flex justify-between items-center border-b border-slate-800 text-xs font-semibold text-slate-400">
        <span className="font-mono text-[11px] sm:text-xs">{match.matchCode}</span>
        <div className="flex items-center gap-1.5">
          {isLive && (
            <span className="flex items-center gap-1 text-[9px] sm:text-[10px] text-amber-400 bg-amber-950/60 border border-amber-500/30 px-1.5 py-0.2 rounded font-bold uppercase">
              <Flame size={10} /> Live
            </span>
          )}
          <span className="text-[9px] sm:text-[10px] bg-slate-800 text-cyan-400 px-2 py-0.5 rounded font-mono font-bold">
            BO{match.bestOf || 3}
          </span>
        </div>
      </div>
      ...
      <div className="p-2 sm:p-2.5 space-y-1.5 sm:space-y-2">
        {/* Team A */}
        <div className={`flex justify-between items-center px-2 sm:px-2.5 py-1.5 rounded-lg ${
          isTeamAWinner 
          ...
          <button
            ...
            className="truncate max-w-[110px] sm:max-w-[130px] text-xs text-left hover:text-cyan-400 transition flex items-center gap-1 group active:opacity-75"
          ...
          <div className="flex items-center gap-1.5">
            <span className={`font-mono text-xs sm:text-sm px-2 py-0.5 rounded ${
              ...
              <div className="flex items-center gap-1 pl-1">
                {match.scoreA > 0 && (
                  <button
                    ...
                    className="w-6 h-6 flex items-center justify-center bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white rounded text-xs active:scale-90 transition"
                    title="Deduct point"
                  >
                    <Minus size={11} />
                  </button>
                ...
                {match.scoreA < maxScore && (
                  <button
                    onClick={() => onScoreChange(match.matchCode, match.scoreA + 1, match.scoreB)}
                    className="w-6 h-6 flex items-center justify-center bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-black rounded text-xs shadow-md shadow-cyan-500/20 active:scale-90 transition"
                    title="Add point"
                  >
      ...
      {match.status === 'COMPLETED' && (
        <div className="bg-cyan-500/10 text-cyan-400 text-[10px] sm:text-[11px] py-1 text-center font-medium border-t border-cyan-500/20 flex items-center justify-center gap-1">
